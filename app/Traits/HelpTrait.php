...
trait HelpTrait
{
    ...
    public static function getOrganisationAdministrators()
    {
        $admins = DB::table('users')
            ...
            ->get()->toArray();
        $president         = collect($admins)->filter(function ($admin) {
            return $admin->role_name == Roles::PRESIDENT;
        });
        $fin_sec           = collect($admins)->filter(function ($admin) {
            return $admin->role_name == Roles::FINANCIAL_SECRETARY;
        });
        $treasurer         = collect($admins)->filter(function ($admin) {
            return $admin->role_name == Roles::TREASURER;
        });
    ...
    {
        $total_payment_item_amount = $payment_item->is_range ? $payment_item->start_amount : $payment_item->amount;
    ...
    {
        $payment_amount = $item->is_range ? $item->start_amount : $item->amount;
        if ($item->frequency == PaymentItemFrequency::QUARTERLY) {
            ...
            $total_amount = count($quarters) * $payment_amount;
            ...
            $total_amount = count($months) * $payment_amount;
        } else {
            ...
            case PaymentItemFrequency::YEARLY:
                switch ($payment_item->type) {
                    ...
                    case PaymentItemType::A_MEMBER:
                        ...
                        $amount = in_array($member->id, $reference_array) ? $payment_amount : 0;
                        break;
                    case PaymentItemType::ALL_MEMBERS:
                        $amount = $payment_amount;
                        break;
