...
@section('section')
            ...
            @foreach ($contributions as $key => $contribution)
                <tr style="border: 1px solid black; font-size: smaller">
                    ...
                    @elseif($contribution->payment_item_frequency == \App\Constants\PaymentItemFrequency::MONTHLY)
                        <td style="border: 1px solid black; padding: 3px; text-align: center">
                            {{ $contribution->month_name }}</td>
                    @else
                        <td style="border: 1px solid black; padding: 3px; text-align: center">
                            {{ str_replace('_', ' ', $contribution->payment_item_frequency) }}</td>
                    @endif
                    <td style="border: 1px solid black; padding: 3px; text-align: center">
    ...
    <div style="margin-top: 4px;">
        @if (
            $paymentItem->frequency == \App\Constants\PaymentItemFrequency::MONTHLY ||
                $paymentItem->frequency == \App\Constants\PaymentItemFrequency::QUARTERLY)
            ...
            <div class="row" style="border: 1px solid black">
                ...
                @if (!$paymentItem->is_range)
                    <div class="activity_summary_end">
                        {{ number_format($paymentItem->start_amount) }} XAF
                    </div>
                @else
                    <div class="activity_summary_end">
                        {{ number_format($paymentItem->start_amount) }} - {{ number_format($paymentItem->end_amount) }}
                        XAF
                    </div>
                @endif
            </div>
